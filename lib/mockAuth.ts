...
// Mock blogs data (mutable array for testing purposes)
const initialMockBlogs: Blog[] = [
    {
        _id: "1",
...
let mockBlogs = [...initialMockBlogs];
...
export const mockAuth = {
    ...
    createBlog(blogData: BlogData) {
        const newBlog = {
            ...
            createdAt: new Date(),
            publishedAt:
                blogData.status === "published" ? new Date() : undefined,
        };
    ...
    // Update blog
    updateBlog(id: string, blogData: Partial<BlogData>) {
    ...
    getStats() {
        return {
            ...
            published: mockBlogs.filter((blog) => blog.status === "published")
                .length,
            draft: mockBlogs.filter((blog) => blog.status === "draft").length,
            archived: mockBlogs.filter((blog) => blog.status === "archived")
                .length,
        };
    },
};
